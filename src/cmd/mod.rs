...
GitHub: https://github.com/chawyehsu/rift/issues", crate_description!()),
...
pub fn start() -> Result<()> {
    ...
    let user_agent = format!("Scoop/1.0 (+https://scoop.sh/) Rift/{}", crate_version!());
...
fn setup_logger(level_filter: LevelFilter) -> Result<()> {
    ...
    // The custom `RIFT_LOG_LEVEL` environment variable was introduced to set the
